...
from thesis.corpus_build.acquisition import download_pmc_xml as dl
from thesis.paths import PUBMED_DIR
...
class SafetyTests(DownloaderTestCase):
    def test_refuses_to_write_under_pubmed(self) -> None:
        with self.assertRaises(SystemExit) as caught:
            dl.assert_safe_output_dir(PUBMED_DIR / "x", self.inventory)
        self.assertIn("never write under data/pubmed", str(caught.exception))
