...
`data/pmc/inventory/pmc_oa_inventory.csv` remains the immutable, authoritative record and was
...
```bash
python -m thesis.corpus_build.acquisition.download_pmc_xml \\
    --inventory data/pmc/inventory/pmc_oa_inventory_reconciled_2026-09-02.csv
```
...
- Derived from `data/pmc/inventory/pmc_oa_inventory.csv` (27,508 records, 31 Aug 2026).
...
- Evidence of the failures is preserved in `data/pmc/fulltext/failures.csv` and
  `data/pmc/fulltext/manifest.csv` on the `acquire-pmc-xml` branch.
