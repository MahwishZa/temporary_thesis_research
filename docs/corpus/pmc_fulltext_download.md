...
The acquisition stage `download_pmc_xml` reads the validated inventory
`data/pmc/inventory/pmc_oa_inventory.csv`,
...
**Never touches:** anything under `data/pubmed/`, or the inventory itself. The
inventory is opened read-only; `data/pubmed/` is not opened at all. A runtime
guard refuses to start if the output directory is inside `data/pubmed/` or beside the
inventory.
...
```bash
python -m thesis.corpus_build.acquisition.download_pmc_xml --list-only  # plan only; no network requests
python -m thesis.corpus_build.acquisition.download_pmc_xml --limit 5    # small test run
python -m thesis.corpus_build.acquisition.download_pmc_xml              # full run (deliberate, no flag)
python -m thesis.corpus_build.acquisition.download_pmc_xml              # again to resume or retry failures
```
...
`data/pmc/fulltext/xml/` should **not** be committed. The manifest, failures file and
...
`data/pubmed/` or any request for a non-XML URL.
