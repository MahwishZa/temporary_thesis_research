#!/usr/bin/env python3
"""Retrieval service: a thin, provenance-preserving facade over ``thesis.corpus_build.indexing.retrieve``.

``thesis.corpus_build.indexing.retrieve`` is the approved implementation and is not reimplemented here.
...
class RetrievalService:
    ...
    def encode(self, text: str) -> Sequence[float]:
        ...
        them. ``thesis.corpus_build.embedding.embed_chunks.get_encoder("medcpt", ...)`` returns the *article*
        ...
        if self._query_encoder is None:  # pragma: no cover - needs weights
            # lazy: torch + transformers + weights
            from .corpus_build.embedding import embed_chunks
    ...
    def save(self, result: RetrievalResult, directory: str) -> str:
        ...
        ``thesis.corpus_build.indexing.retrieve.save_candidates`` records the index identity and the
