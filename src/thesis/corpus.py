...
The corpus is an established asset: built by ``thesis.corpus_build.chunking.build_chunks``, embedded by
``thesis.corpus_build.embedding.embed_chunks``, verified by ``thesis.corpus_build.qc.verify_index``. This module does
        ...
        if expected_digest:
            if not actual_digest:
                raise CorpusError(
                    ...
                    "that recorded one; rebuild with thesis.corpus_build.chunking.build_chunks."
                )
        ...
        if wants_index:
            if not self.index_meta:
                raise CorpusError(
                    ...
                    "thesis.corpus_build.embedding.embed_chunks, or set corpus.require_production_index=false for a "
    ...
    def iter_chunks(self, limit: Optional[int] = None) -> Iterator[Dict[str, Any]]:
        ...
        if not os.path.exists(self.chunks_path):
            raise CorpusError(
                ...
                "deterministically with thesis.corpus_build.chunking.build_chunks."
            )
        count = 0
