...
Reads the XML files in data/pmc/fulltext/xml/, joins acquisition provenance from
data/pmc/fulltext/manifest.csv, and writes JSONL -- one article per line.
...
Usage:
    python -m thesis.corpus_build.parsing.parse_pmc_xml --pmcids PMC9277667,PMC11868538   # sample
    python -m thesis.corpus_build.parsing.parse_pmc_xml --limit 10                        # first N
    python -m thesis.corpus_build.parsing.parse_pmc_xml                                   # everything
...
DEFAULT_XML_DIR = PMC_XML_DIR
DEFAULT_MANIFEST = PMC_MANIFEST_CSV
DEFAULT_OUTPUT = PMC_ARTICLES_JSONL
...
def assert_safe_output(output: Path, xml_dir: Path) -> None:
    ...
    if PUBMED_DIR.resolve() in resolved.parents:
        raise SystemExit("ERROR: this tool must never write under data/pubmed/.")
