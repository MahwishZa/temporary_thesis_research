...
Reads data/pmc/chunks/chunks.jsonl (frozen, unmodified) and writes an additive index
layer under data/pmc/index/:
...
from thesis.paths import PMC_CHUNKS_JSONL, PMC_INDEX_DIR

DEFAULT_CHUNKS = PMC_CHUNKS_JSONL
DEFAULT_OUT = PMC_INDEX_DIR
...
def embed_text_of(chunk: dict) -> str:
    ...
    The rule is imported from the chunking stage rather than restated, so what
    is indexed and what is scored can never drift apart. The import is resolved
    once and cached; it used to run per chunk, 781k times over a production run.
    """
    global _COMPOSE_EMBED_TEXT
    if _COMPOSE_EMBED_TEXT is None:
        from thesis.corpus_build.chunking.build_chunks import compose_embed_text
