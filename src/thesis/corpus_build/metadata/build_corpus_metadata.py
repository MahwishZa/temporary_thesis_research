...
It reads them and writes derived overlay files under data/pmc/metadata/ and the
externally-added currency-pack layer under data/pmc/currency_pack/.

    M1  currency pack        -> data/pmc/metadata/currency_pack.csv
                                data/pmc/currency_pack/{xml,parsed}/PMC13082890.*
    M2  CPG-AD layer (seed)  -> data/pmc/metadata/cpg_registry.csv   (see the STOP note)
    M3  canonical dates      -> data/pmc/metadata/canonical_dates.csv
    M4  corpus policy        -> data/pmc/metadata/corpus_policy.csv
...
from thesis.paths import (
    PMC_ARTICLES_JSONL,
    PMC_CURRENCY_PACK_DIR,
    PMC_MANIFEST_CSV,
    PMC_METADATA_DIR,
    PMC_QC_REPORT_POSTFIX,
    PUBMED_RESULTS_CSV,
)

csv.field_size_limit(2**31 - 1 if sys.maxsize > 2**32 else 2**27)

DEFAULT_MANIFEST = PMC_MANIFEST_CSV
DEFAULT_PUBMED = PUBMED_RESULTS_CSV
DEFAULT_ARTICLES = PMC_ARTICLES_JSONL
DEFAULT_REPORT = PMC_QC_REPORT_POSTFIX
DEFAULT_OUT = PMC_METADATA_DIR
DEFAULT_CURRENCY_DIR = PMC_CURRENCY_PACK_DIR
...
def ingest_cochrane(currency_dir: Path, *, allow_fetch: bool) -> dict:
    ...
    from thesis.corpus_build.parsing import parse_pmc_xml as pp
