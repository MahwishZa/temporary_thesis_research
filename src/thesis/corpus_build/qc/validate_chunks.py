...
Read-only. Exits non-zero if any invariant fails, so it can gate the next stage.
    ...
    python -m thesis.corpus_build.qc.validate_chunks                 # after building chunks
    python -m thesis.corpus_build.qc.validate_chunks --pubmed <csv>  # if PubMed is not smudged
...
from thesis.paths import PMC_CHUNKS_JSONL, PMC_METADATA_DIR, REPO_ROOT

csv.field_size_limit(2**31 - 1 if sys.maxsize > 2**32 else 2**27)

DEFAULT_CHUNKS = PMC_CHUNKS_JSONL
DEFAULT_META = PMC_METADATA_DIR
...
def main(argv=None) -> int:
    ...
    xml_tracked = [t for t in tracked if t.startswith("data/pmc/fulltext/xml/")]
