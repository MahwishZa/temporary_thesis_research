...
Read-only over the corpus: opens data/pmc/parsed/articles.jsonl, data/pmc/fulltext/manifest.csv
...
parser defect; the absence of one is source variability.
    ...
    python -m thesis.corpus_build.qc.qc_investigate                       # full report
    python -m thesis.corpus_build.qc.qc_investigate --no-xml              # skip raw-XML checks
    python -m thesis.corpus_build.qc.qc_investigate --output docs/corpus/qc/qc.md --force
...
DEFAULT_JSONL = PMC_ARTICLES_JSONL
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path,
                        default=QC_REPORTS_DIR / f"pmc_qc_report_{date.today().isoformat()}.md")
