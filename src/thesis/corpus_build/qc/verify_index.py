...
Run this after thesis.corpus_build.embedding.embed_chunks finishes, before anything retrieves over the
result. It answers one question: does data/pmc/index/ actually correspond, row for
row, to the frozen chunk layer it claims to be built from?

    python -m thesis.corpus_build.qc.verify_index
...
def verify(index_dir: Path, chunks_path: Path, skip_duplicates: bool = True) -> int:
    ...
    for path in (vec_path, man_path, meta_path):
        if not report.check(path.exists(), f"{path.name} exists", str(path)):
            print("\nFAILED: the index is incomplete. Run thesis.corpus_build.embedding.embed_chunks first.")
            return 1
