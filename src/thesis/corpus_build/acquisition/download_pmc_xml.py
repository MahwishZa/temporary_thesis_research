...
Reads the validated inventory at data/pmc/inventory/pmc_oa_inventory.csv, selects the records
...
WHAT THIS NEVER TOUCHES
    Anything under data/pubmed/, and the inventory itself. Both are opened
    read-only (the inventory) or not at all, and a runtime guard refuses to write
    into either.
...
Usage:
    python -m thesis.corpus_build.acquisition.download_pmc_xml --list-only      # plan, no network at all
    python -m thesis.corpus_build.acquisition.download_pmc_xml --limit 5        # small test run
    python -m thesis.corpus_build.acquisition.download_pmc_xml                  # full run (deliberate)
    python -m thesis.corpus_build.acquisition.download_pmc_xml                  # run again to resume
...
from thesis.paths import PMC_FULLTEXT_DIR, PMC_INVENTORY_CSV, PUBMED_DIR

DEFAULT_INVENTORY = PMC_INVENTORY_CSV
DEFAULT_OUTPUT_DIR = PMC_FULLTEXT_DIR
...
# ---------------------------------------------------------------------------
# Output layout, and the guard that keeps us out of data/pubmed/
# ---------------------------------------------------------------------------
