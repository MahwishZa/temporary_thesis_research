...
For every PMCID in data/pubmed/pubmed_results.csv this asks the PMC Cloud Service
whether the article is in the PMC open-access dataset and, if so, under what
licence and where its files live. It writes data/pmc/inventory/pmc_oa_inventory.csv.

It does NOT download any full text -- only the small per-article metadata
record (roughly 1.5 KB each). It never writes to anything under data/pubmed/.
...
Usage:
    python -m thesis.corpus_build.acquisition.inventory_pmc_oa --limit 25    # small test first
    python -m thesis.corpus_build.acquisition.inventory_pmc_oa               # the whole set
    python -m thesis.corpus_build.acquisition.inventory_pmc_oa               # run again to resume
...
from thesis.paths import (
    PMC_INVENTORY_CSV,
    PMC_INVENTORY_FAILURES_CSV,
    PUBMED_RESULTS_CSV,
)

DEFAULT_INPUT = PUBMED_RESULTS_CSV
DEFAULT_OUTPUT = PMC_INVENTORY_CSV
DEFAULT_FAILURES = PMC_INVENTORY_FAILURES_CSV
