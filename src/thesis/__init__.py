...
turns the repository's already-validated parts into one executable pipeline:
    ...
    research query
      ...
      -> retrieval over the frozen PMC corpus    thesis.retrieval  -> thesis.corpus_build.indexing.retrieve
...
* **Nothing here modifies ``rag2/rag2/**``.** The reproduced baseline is called
  ...
  guards that tree; ``tests/unit/test_condition_isolation.py`` guards this one.
...
* **One corpus, one query set, one evaluation protocol across conditions.**
  ...
  see. ``thesis.corpus_build.indexing.retrieve``'s fingerprinted candidate replay is what makes that
